...
START_TEST(add_header) {
    SMFHeader_T *hdr;

    // Create a new header
    fail_unless(smf_message_add_header(msg, "X-Foo", "foo") == 0);
    fail_unless((hdr = smf_message_get_header(msg, "X-Foo")) != NULL);
    ck_assert_int_eq(smf_header_get_count(hdr), 1);
    ck_assert_str_eq(smf_header_get_name(hdr), "X-Foo");
    ck_assert_str_eq(smf_header_get_value(hdr, 0), "foo");
    
    // Update already existing header
    fail_unless(smf_message_add_header(msg, "X-Foo", "bar") == 0);
    fail_unless((hdr = smf_message_get_header(msg, "X-Foo")) != NULL);
    ck_assert_int_eq(smf_header_get_count(hdr), 2);
    ck_assert_str_eq(smf_header_get_name(hdr), "X-Foo");
    ck_assert_str_eq(smf_header_get_value(hdr, 0), "foo");
    ck_assert_str_eq(smf_header_get_value(hdr, 1), "bar");
}
END_TEST
